...
import { BarChart3, ChevronDown, ChevronRight, Euro } from 'lucide-react';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setOrgRole(role);
        if (!['admin', 'directeur', 'service_formation'].includes(role)) {
                      ...
                      return details.map((d) => (
                        <tr key={`${s.id}-${d.id}`} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 py-2">
                            {d.type === 'autre' ? (d.intitule || 'Formation autre') : (d.formation_libelle || '—')}
                          </td>
